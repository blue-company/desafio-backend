        ...
        let randomNumber = Math.floor(Math.random() * 1000000)
         ejs.renderFile(path.join(__dirname,'../views/pdf.ejs'), {username, doctorName, consultationTime, consultationDate, speciality},  (err, html) => {
            if(err) {
                ...
                console.log(html)
                pdf.create(html, {}).toFile(path.join(__dirname, `../views/consultations/${username.replace(/\s+/g, '')}_${user_id}/${username.replace(/\s+/g, '')}_${doctorName.replace(/\s+/g, '')}_consulta_${randomNumber}.pdf`), (err, res) => {
                    if(err) {
            ...
            let file = `/consultations/${username.replace(/\s+/g, '')}_${user_id}/${username.replace(/\s+/g, '')}_${doctorName.replace(/\s+/g, '')}_consulta_${randomNumber}.pdf`
            return file
